...
#define op_arithI(L,iop,fop) {  \
  ...
  if (ttisstring(v1)) {  \
    char buff[64];  \
    sprintf(buff, LUA_INTEGER_FMT, (LUAI_UACINT)imm);  \
    TString *ts = luaS_new(L, buff);  \
    setobj2s(L, ra, v1);  \
    setsvalue2s(L, ra + 1, ts);  \
    L->top.p = ra + 2;  \
    pc++; luaV_concat(L, 2);  \
  }  \
  else if (ttisinteger(v1)) {  \
...
#define op_arith_aux(L,v1,v2,iop,fop) {  \
  int t1 = ttisinteger(v1);  \
  int t2 = ttisinteger(v2);  \
  if (t1 && t2) {  \
    StkId ra = RA(i); \
    ...
    StkId ra = RA(i);  \
    lua_Number n1 = t1 ? cast_num(ivalue(v1)) : fltvalue(v1);  \
    lua_Number n2 = t2 ? cast_num(ivalue(v2)) : fltvalue(v2);  \
    pc++; setfltvalue(s2v(ra), fop(L, n1, n2));  \
  }  \
  else {  \
    StkId ra = RA(i);  \
    lua_Number n1 = fltvalue(v1); lua_Number n2 = fltvalue(v2);  \
    pc++; setfltvalue(s2v(ra), fop(L, n1, n2));  \
  }}
...
#define op_arithK(L,iop,fop) {  \
  ...
  if (ttisstring(v1) || ttisstring(v2)) {  \
    StkId ra = RA(i);  \
    if (!ttisstring(v1)) luaO_tostring(L, v1);  \
    if (!ttisstring(v2)) luaO_tostring(L, v2);  \
    setobj2s(L, ra, v1);  \
    setobj2s(L, ra + 1, v2);  \
    L->top.p = ra + 2;  \
    luaV_concat(L, 2);  \
  ...
  for (;;) {
    ...
    vmdispatch (GET_OPCODE(i)) {
      ...
      vmcase(OP_ADD) {
        ...
        TValue *rb = vRB(i);
        TValue *rc = vRC(i);
        if (ttisstring(rb) || ttisstring(rc)) {
      ...
      vmcase(OP_ADD_I) {
        lua_Integer ib = ivalue(vRB(i));
        lua_Integer ic = ivalue(vRC(i));
        setivalue(vRA(i), ib + ic);
        vmbreak;
      }
      vmcase(OP_ADD_F) {
        lua_Number nb = fltvalue(vRB(i));
        lua_Number nc = fltvalue(vRC(i));
        setfltvalue(vRA(i), nb + nc);
        vmbreak;
      }
      vmcase(OP_SUB_I) {
        lua_Integer ib = ivalue(vRB(i));
      ...
      vmcase(OP_MMBIN) {
        ...
        StkId result = RA(pi);
        /* Mos: Allow type-specialized instructions (OP_ADD_I, OP_ADD_F, etc.) */
        OpCode op = GET_OPCODE(pi);
        lua_assert((OP_ADD <= op && op <= OP_SHR) ||
                   (OP_ADD_I <= op && op <= OP_DIV_F));
