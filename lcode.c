...
void luaK_dischargevars (FuncState *fs, expdesc *e) {
  switch (e->k) {
    ...
    case VLOCAL: {  /* already in a register */
      ...
      /* Mos: Type info is preserved in e->etype */
      break;
    }
...
/* Mos: Same as finishbinexpval but skips metamethod instruction for typed integers */
static void finishbinexpval_typed (FuncState *fs, expdesc *e1, expdesc *e2,
                                   OpCode op, int v2, int line) {
  int v1 = luaK_exp2anyreg(fs, e1);
  int pc = luaK_codeABCk(fs, op, 0, v1, v2, 0);
  /* Mos: Generate a dummy MMBIN instruction so VM's pc++ doesn't skip next real instruction */
  luaK_codeABC(fs, OP_MMBIN, 0, 0, 0);
  freeexps(fs, e1, e2);
  e1->u.info = pc;
  e1->k = VRELOC;
  luaK_fixline(fs, line);
...
static void codebinexpval (FuncState *fs, BinOpr opr,
  ...
  /* Mos: Use type-specialized instructions based on expression types */
  if (opr >= OPR_ADD && opr <= OPR_DIV) {
    /* Check if both operands are known to be integers */
    int both_int = 0;
    int both_float = 0;
    
    if (isKint(e1) && isKint(e2)) {
      both_int = 1;
    }
    else if (e1->k == VKFLT && e2->k == VKFLT && !hasjumps(e1) && !hasjumps(e2)) {
      both_float = 1;
    }
    else if (e1->etype == MOS_TYPE_INT && e2->etype == MOS_TYPE_INT) {
      both_int = 1;
    }
    else if (e1->etype == MOS_TYPE_FLOAT && e2->etype == MOS_TYPE_FLOAT) {
      both_float = 1;
    }
    
    if (both_int) {
      /* Both are integers - use type-specialized instruction */
      OpCode typed_op;
      int needs_mmbin = 0;
      switch (opr) {
        case OPR_ADD: typed_op = OP_ADD_I; break;
        case OPR_SUB: typed_op = OP_SUB_I; break;
        case OPR_MUL: typed_op = OP_MUL_I; break;
        case OPR_DIV: typed_op = OP_DIV_I; break;
        case OPR_MOD: typed_op = OP_MOD; needs_mmbin = 1; break;
        case OPR_POW: typed_op = OP_POW; needs_mmbin = 1; break;
        default: typed_op = op; break;
      }
      int v1 = luaK_exp2anyreg(fs, e1);
      int pc = luaK_codeABCk(fs, typed_op, 0, v1, v2, 0);
      /* Mos: Generate MMBIN for instructions that use pc++ (MOD, POW) */
      if (needs_mmbin)
        luaK_codeABC(fs, OP_MMBIN, 0, 0, 0);
      freeexps(fs, e1, e2);
      e1->u.info = pc;
      e1->k = VRELOC;
      e1->etype = MOS_TYPE_INT;  /* Mos: Result type is integer */
      luaK_fixline(fs, line);
      return;
    }
    else if (both_float) {
      /* Both are floats - use type-specialized instruction */
      OpCode typed_op;
      int needs_mmbin = 0;
      switch (opr) {
        case OPR_ADD: typed_op = OP_ADD_F; break;
        case OPR_SUB: typed_op = OP_SUB_F; break;
        case OPR_MUL: typed_op = OP_MUL_F; break;
        case OPR_DIV: typed_op = OP_DIV_F; break;
        case OPR_MOD: typed_op = OP_MOD; needs_mmbin = 1; break;
        case OPR_POW: typed_op = OP_POW; needs_mmbin = 1; break;
        default: typed_op = op; break;
      }
      ...
      e1->u.info = pc;
      e1->k = VRELOC;
      e1->etype = MOS_TYPE_FLOAT;  /* Mos: Result type is float */
      luaK_fixline(fs, line);
      return;
    }
  }
...
static void codebini (FuncState *fs, OpCode op,
  ...
  if (e1->etype == MOS_TYPE_INT) {
    finishbinexpval_typed(fs, e1, e2, op, v2, line);
    e1->etype = MOS_TYPE_INT;
  } else {
...
static int finishbinexpneg (FuncState *fs, expdesc *e1, expdesc *e2,
  ...
  if (!isKint(e2))
    ...
    if (!(fitsC(i2) && fitsC(-i2)))
      ...
      int v2 = cast_int(i2);
      if (e1->etype == MOS_TYPE_INT) {
        finishbinexpval_typed(fs, e1, e2, op, int2sC(-v2), line);
        e1->etype = MOS_TYPE_INT;
      } else {
...
static void codecommutative (FuncState *fs, BinOpr op,
  ...
  /* Mos: Check if both operands are strings, use concatenation */
  if (op == OPR_ADD && e1->k == VKSTR && e2->k == VKSTR) {
    luaK_exp2nextreg(fs, e1);
    luaK_exp2nextreg(fs, e2);
    luaK_codeABC(fs, OP_CONCAT, e1->u.info, 2, 0);
  }
  else if (op == OPR_ADD && isSCint(e2))  /* immediate operand? */
...
void luaK_prefix (FuncState *fs, UnOpr opr, expdesc *e, int line) {
  static const expdesc ef = {VKINT, {0}, NO_JUMP, NO_JUMP, MOS_TYPE_INT};
  luaK_dischargevars(fs, e);
  switch (opr) {
    ...
    case OPR_LEN:
      ...
      /* Mos: Set result type for unary operations */
      if (opr == OPR_LEN) {
        e->etype = MOS_TYPE_INT;  /* length operator returns int */
      }
      /* OPR_MINUS and OPR_BNOT preserve the operand type */
      break;
...
void luaK_posfix (FuncState *fs, BinOpr opr,
  ...
  switch (opr) {
    ...
    case OPR_CONCAT: {  /* e1 .. e2 */
      ...
      e1->etype = MOS_TYPE_STRING;  /* Mos: Concatenation result is string */
      break;
    }
    case OPR_ADD: case OPR_MUL: {
      codecommutative(fs, opr, e1, e2, line);
      /* Mos: Set result type for + operator */
      if (opr == OPR_ADD) {
        if (e1->etype == MOS_TYPE_STRING || e2->etype == MOS_TYPE_STRING)
          e1->etype = MOS_TYPE_STRING;  /* String + anything = String */
        else if (e1->etype == MOS_TYPE_FLOAT || e2->etype == MOS_TYPE_FLOAT)
          e1->etype = MOS_TYPE_FLOAT;  /* float + int = float */
        /* else type already set by codebinexpval or stays as-is */
      }
      break;
    }
    case OPR_SUB: {
    ...
    case OPR_BAND: case OPR_BOR: case OPR_BXOR: {
      ...
      e1->etype = MOS_TYPE_INT;  /* Mos: Bitwise operations return int */
      break;
    }
    case OPR_SHL: {
      if (isSCint(e2))
      ...
      e1->etype = MOS_TYPE_INT;  /* Mos: Shift operations return int */
      break;
    }
    case OPR_SHR: {
      if (isSCint(e2))
    ...
    case OPR_EQ: case OPR_NE: {
      codeeq(fs, opr, e1, e2);
      e1->etype = MOS_TYPE_BOOL;  /* Mos: Comparison result is bool */
      break;
    }
    case OPR_GT: case OPR_GE: {
      /* '(a > b)' <=> '(b < a)';  '(a >= b)' <=> '(b >= a)' */
