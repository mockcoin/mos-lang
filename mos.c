/*
** $Id: mos.c $
** Mos stand-alone interpreter
** See Copyright Notice in mos.h
*/
...
#if !defined(MOS_PROGNAME)
#define MOS_PROGNAME		"mos"
#endif

#if !defined(MOS_INIT_VAR)
#define MOS_INIT_VAR		"MOS_INIT"
#endif

#define MOS_INITVARVERSION	MOS_INIT_VAR MOS_VERSUFFIX


static lua_State *globalL = NULL;

static const char *progname = MOS_PROGNAME;
...
static void print_version (void) {
  lua_writestring(MOS_COPYRIGHT, strlen(MOS_COPYRIGHT));
  lua_writeline();
}
...
static int handle_mosinit (lua_State *L) {
  const char *name = "=" MOS_INITVARVERSION;
...
static int pmain (lua_State *L) {
  ...
  if (!(args & has_E)) {  /* no option '-E'? */
    if (handle_mosinit(L) != LUA_OK)  /* run MOS_INIT */
      return 0;  /* error running MOS_INIT */
  }
